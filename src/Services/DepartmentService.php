...
class DepartmentService extends BaseService
{
    ...
    public function sync($department): bool
    {
        if ($department?->deleted_at ?? false) {
            ...
            Log::info("Delete user sso_id: " . $department->code . "  Success");
        ...
        if (!$departmentModel) {
            if (config('database.default') == 'mongodb') {
                $departmentModel = new DepartmentNoSQL();
            }
            else {
        ...
        $departmentModel->unit_id = $department->unit_id;
        ...
        try {
            ...
            Log::info("Update user sso_id: " . $department->code . "  Success");
        } catch (Exception $e) {
