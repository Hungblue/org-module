...
class UserService extends BaseService
{
    ...
    public function sync($user): bool
    {
        ...
        if ($user->user_department) {
            $department                 = is_array($user->user_department) ? (object)$user->user_department
                : $user->user_department;
            $userModel->department      = $department->department->name ?? '';
            $userModel->department_code = $department->department->code ?? '';
            $userModel->unit            = '';
            if ($department->department?->unit) {
                $unit            = is_array($department->department->unit) ? (object)$department->department->unit
